...
BOT_TOKEN = "8664489723:AAFNxzn-YJB829RtV0CLHmoUPzeS-psC3jk"
OWNER_ID  = 6784382795
...
# ── Free Key System (hosted key panel) ─────────────────
import os
KEY_API_BASE   = os.getenv("KEY_API_BASE", "https://project--2dbd6138-c43a-4b57-8569-8149dc7e8412.lovable.app")
...
def load_store() -> Dict[str, Any]:
    try:
        if STORE_PATH.exists():
            ...
            data["vip_users"]     = list({_list(x) for x in data.get("vip_users", [])})
...
AWAITING_KEY: Dict[int, bool] = {}


async def _key_api(path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    if not BOT_API_SECRET:
        return {"ok": False, "reason": "not_configured"}
    url = f"{KEY_API_BASE.rstrip('/')}/api/public/keys/{path}"
    try:
        ...
        log.error(f"key api error: {e}")
        return {"ok": False, "reason": "network"}


async def api_redeem_key(code: str, uid: int, name: str) -> Dict[str, Any]:
    return await _key_api("redeem", {"code": code, "telegram_id": uid, "telegram_name": name})


async def api_key_status(uid: int) -> Dict[str, Any]:
    return await _key_api("status", {"telegram_id": uid})


def grant_from_key(uid: int, expires_at: str, name: str = "") -> None:
    """Give access to a user after a valid key redeem."""
    store_allow(uid, name)
    try:
        exp = datetime.fromisoformat(expires_at.replace("Z", "+00:00")).replace(tzinfo=None)
    except Exception:
        exp = datetime.now() + timedelta(days=3)
    EXPIRY[str(uid)] = exp.isoformat()
    STORE["expiry"] = EXPIRY
    save_store(STORE)
...
class T:
    ...
    def welcome(name, username, uid):
        ...
        if exp:
            try:
                left = datetime.fromisoformat(exp) - now
                d, hrs = left.days, left.seconds // 3600
                if left.total_seconds() > 0:
                    plan_line = f"  │ ⏳ Access: {d}d {hrs}h left\n"
            except Exception:
                pass
        return (
            f"{B}\n"
            ...
            f"{B}\n\n"
            f"  {greet}, <b>{escape(name)}</b>! 👋\n"
            f"  Welcome to the most-\n"
            f"  fastest CPM tool in Telegram.\n\n"
            f"  ╭──── 𝗬𝗢𝗨𝗥 𝗣𝗥𝗢𝗙𝗜𝗟𝗘 ────╮\n"
            f"  │ 👤 {escape(name)}\n"
            f"  │ 📱 @{escape(username) if username else 'N/A'}\n"
            f"  │ 🆔 <code>{uid}</code>\n"
            f"{plan_line}"
            ...
            f"  ╭──── 𝗪𝗛𝗔𝗧 𝗬𝗢𝗨 𝗚𝗘𝗧 ────╮\n"
            f"  │ 💰 Money & coins boost\n"
            f"  │ 🔓 Unlock cars, wheels, skins\n"
            f"  │ 🏆 Rank & level editor\n"
            f"  │ ⚡ Instant, safe & auto-save\n"
            f"  ╰──────────────────────╯\n\n"
            f"  ▸ Tap <b>Sign In</b> and use your CPM\n"
            f"    email & password to start.\n"
            f"  ⚠️ Never share your key or password."
        )
    ...
    def no_access():
        return (
            f"{B}\n  🔒  𝗔𝗖𝗖𝗘𝗦𝗦 𝗥𝗘𝗤𝗨𝗜𝗥𝗘𝗗\n{B}\n\n"
            "  You don’t have access to the tool yet.\n\n"
            "  ╭──── 𝗢𝗣𝗧𝗜𝗢𝗡𝗦 ────╮\n"
            "  │ 🔑 Redeem a FREE key\n"
            "  │ 📨 Request access sa admin\n"
            "  ╰─────────────────╯\n\n"
            "  ▸ Do you have key? Tap <b>Redeem Free Key</b>\n"
            "    o i-type: <code>/key YOUR-KEY</code>"
        )
...
class K:
    ...
    def no_access():
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔑 Redeem Free Key", callback_data="redeem_key")],
...
def _key_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Try again",     callback_data="redeem_key")],
        [InlineKeyboardButton(text="📨 Request Access", callback_data="send_request")],
    ])


async def _process_key(msg: Message, code: str):
    uid  = msg.from_user.id
    name = msg.from_user.full_name
    AWAITING_KEY.pop(uid, None)

    if is_banned(uid):
        await msg.answer(T.banned()); return

    res = await api_redeem_key(code.strip(), uid, name)

    if res.get("ok"):
        exp = res.get("expires_at", "")
        grant_from_key(uid, exp, name)
        try:
            left = datetime.fromisoformat(exp.replace("Z", "+00:00")).replace(tzinfo=None) - datetime.now()
            left_txt = f"{left.days}d {left.seconds // 3600}h"
        except Exception:
            left_txt = "—"
        slots = res.get("slots_left")
        slot_txt = f"  │ 👥 Slots left: {slots}\n" if slots is not None else ""
        await msg.answer(
            f"{B}\n  ✅  𝗞𝗘𝗬 𝗔𝗖𝗧𝗜𝗩𝗔𝗧𝗘𝗗\n{B}\n\n"
            f"  ╭──── 𝗔𝗖𝗖𝗘𝗦𝗦 ────╮\n"
            f"  │ 🎟 Plan: {res.get('plan','—')}\n"
            f"  │ ⏳ Valid for: {left_txt}\n"
            f"{slot_txt}"
            f"  ╰─────────────────╯\n\n"
            f"  ▸ Send /start to open the tool.",
            reply_markup=K.login())
        return

    reason = res.get("reason", "")
    msgs = {
        "invalid":        "❌ Invalid key. Check mo ulit ang code.",
        "full":           f"🚫 Ubos na ang slots ng key na ito (limit: {res.get('max_users','—')} users).",
        "inactive":       "🚫 Disabled/used-up na ang key na ito.",
        "expired":        "⏰ Expired na ang key mo. Kumuha ng bagong key.",
        "not_configured": "⚙️ Hindi pa naka-setup ang key system. Contact admin.",
        "network":        "🌐 Network error. Try again in a moment.",
        "unauthorized":   "⚙️ Key server rejected the bot. Contact admin.",
    }
    await msg.answer(
        f"{B}\n  ❌  𝗞𝗘𝗬 𝗙𝗔𝗜𝗟𝗘𝗗\n{B}\n\n  {msgs.get(reason, 'Invalid key.')}",
        reply_markup=_key_kb())


@rt.message(Command("key"))
async def cmd_key(msg: Message, state: FSMContext):
    await state.clear()
    parts = (msg.text or "").split(maxsplit=1)
    if len(parts) < 2 or not parts[1].strip():
        AWAITING_KEY[msg.from_user.id] = True
        await msg.answer(
            f"{B}\n  🔑  𝗥𝗘𝗗𝗘𝗘𝗠 𝗙𝗥𝗘𝗘 𝗞𝗘𝗬\n{B}\n\n"
            "  I-send mo lang ang key code dito.\n"
            "  Halimbawa: <code>CPM-ABCD-EFGH-JKLM</code>")
        return
    await _process_key(msg, parts[1])


@rt.message(Command("myaccess"))
async def cmd_myaccess(msg: Message):
    res = await api_key_status(msg.from_user.id)
    if res.get("active"):
        exp = res.get("expires_at", "")
        try:
            left = datetime.fromisoformat(exp.replace("Z", "+00:00")).replace(tzinfo=None) - datetime.now()
        ...
        await msg.answer(
            f"{B}\n  🎟  𝗠𝗬 𝗔𝗖𝗖𝗘𝗦𝗦\n{B}\n\n"
            f"  ◆ Plan:    {res.get('plan','—')}\n"
            f"  ◆ Expires: {left_txt} left")
    else:
        await msg.answer(
            f"{B}\n  🎟  𝗠𝗬 𝗔𝗖𝗖𝗘𝗦𝗦\n{B}\n\n  Wala kang active key.",
            reply_markup=_key_kb())


@rt.callback_query(F.data == "redeem_key")
async def cb_redeem_key(cb: CallbackQuery, state: FSMContext):
    await state.clear()
    AWAITING_KEY[cb.from_user.id] = True
...
@rt.message(F.text)
async def catch_key_text(msg: Message, state: FSMContext):
    if not AWAITING_KEY.get(msg.from_user.id):
        raise SkipHandler
    if await state.get_state() is not None:
        raise SkipHandler
    text = (msg.text or "").strip()
    if text.startswith("/"):
        AWAITING_KEY.pop(msg.from_user.id, None)
        raise SkipHandler
    await _process_key(msg, text)
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="key",    description="🔑 Redeem free key"),
        BotCommand(command="myaccess", description="🎟 My access"),
